...
bool check(int ProcNum, int ProcRank, double *cube, double *cube_old, int incremention){
    double diff_value, max_diff_value;
...
void board_calculations(int ProcNum, int ProcRank, double *cube, int incremention){
    if (ProcRank != 0) {
...
void wait(int ProcNum, int ProcRank, MPI_Request *snd_request, MPI_Request *rcv_request) {
    if (ProcRank != 0) {
...
void non_board_calculations(int ProcNum, int ProcRank, double *cube, int incremention){
...
void SndRcv(int ProcNum, int ProcRank, double *cube, MPI_Request *snd_request, MPI_Request *rcv_request){
    if (ProcRank != 0) {
...
int main(int argc, char* argv[])
{
    ...
    while(true){
        SndRcv(ProcNum, ProcRank, cube, snd_request, rcv_request);//send receive bloc
        non_board_calculations(ProcNum, ProcRank, cube, incremention);//calculate bloc
        wait(ProcNum, ProcRank, snd_request, rcv_request);//wait bloc
        board_calculations(ProcNum, ProcRank, cube, incremention);//calculate bloc 2 (boarders)

        if (check(ProcNum, ProcRank, cube, cube_old, incremention))//check bloc
            break;
